```
Private Score : 174.29713

Public Score  : 306.33700



import os, re, gc, sys, socket, subprocess, time, math, pickle
import pandas as pd
import numpy as np
import torch
...
# --- Fuzzy / multi-key: dùng cho copy-from-train ---
PUNCT_CHARS = r"\.\,\!\?\;\:\(\)\[\]\{\}\"'“”‘’।\|/\\\-"
...
def make_exact_keys(s: str):
    """
    Tạo nhiều key cho exact-lookup:
      - k0: normalize_space(s)
      - k1: bỏ punctuation
      - k2: lowercase
      - k3: chuẩn hóa số thành 0
    """
    s0 = normalize_space(s)
    s1 = normalize_space(re.sub(f"[{PUNCT_CHARS}]", " ", s0))
    s2 = normalize_space(s0.lower())
    s3 = normalize_space(re.sub(r"\d", "0", s0))
    return {s0, s1, s2, s3}
...
# ============================================================
# 3. TRAIN-LOOKUP (MULTI-KEY + FUZZY COPY)
# ============================================================
...
print("Creating train lookup dictionary (for multi-key exact + fuzzy copy)...")
train_lookup  = defaultdict(dict)                      # multi-key exact
...
for (s_lang, t_lang), pairs in bitext_by_dir.items():
    ...
    for s, t in pairs:
        # multi-key exact
        for k in make_exact_keys(s):
            if k and k not in d_exact:
                d_exact[k] = t
...
print(f"Exact lookup example: Hindi->Bhilli has {len(train_lookup[('Hindi','Bhilli')]):,} keys.")

def lookup_train_exact(src_sent, src_lang_c, tgt_lang_c):
    """
    Multi-key exact lookup theo cùng schema với make_exact_keys.
    """
    d = train_lookup.get((src_lang_c, tgt_lang_c), {})
    for k in make_exact_keys(src_sent):
        if k in d:
            return d[k]
    return None

# Hàm fuzzy copy conservative
def fuzzy_copy_from_train(src_norm, src_lang_c, tgt_lang_c, max_len_diff=1):
    """
...
def beam_decode_n_best(src_tokens, lex_prob, lm_score_fn,
                       beam_size=6,        # beam cho SMT
                       lm_w=0.85,
    ...
    for w in src_tokens:
        ...
        if is_copy_token(w):
            ...
            if not dist:
                ...
                lst = sorted(dist.items(), key=lambda x: x[1], reverse=True)[:6]
...
def smt_generate_n_best(smt_model, s, src_lang, tgt_lang, n_best=6):
    """
...
# ============================================================
# 7. MBR RERANKING + POST-PROCESSING
# ============================================================
MBR_GAIN_BLEU_W = 0.6
MBR_GAIN_CHRF_W = 0.4
NLLB_N_BEST     = 12   # dùng làm num_beams & num_return_sequences
SMT_N_BEST      = 6    # SMT N-best cho đa dạng

def calculate_utility(hyp, ref_list):
...
def mbr_rerank_texts(candidates):
    """
...
def postprocess_target(text: str, tgt_lang: str) -> str:
    """
    Post-processing nhẹ để dọn spacing / punctuation.
    Chỉ dùng cho output từ SMT + NLLB (không đụng tới câu copy từ train).
    """
    s = str(text)
    ...
    # Xoá space trước các dấu câu ASCII
    s = re.sub(r"\s+([.,!?;:])", r"\1", s)

    # Dấu câu đặc biệt như '।'
    s = re.sub(r"\s+।", "।", s)

    # Dấu ngoặc
    s = s.replace("( ", "(").replace(" )", ")")

    # Dấu ngoặc kép unicode
    s = s.replace("“ ", "“").replace(" ”", "”")
    s = s.replace("‘ ", "‘").replace(" ’", "’")
...
# ============================================================
@torch.no_grad()
def nllb_generate_n_best(texts, src_lang, tgt_lang, max_new_tokens=256):
    """
    Dùng beam search với:
      - num_beams = NLLB_N_BEST (=12)
      - num_return_sequences = NLLB_N_BEST (=12)
...
if USE_NLLB or USE_SMT:
    ...
    print("\n🚀 Ensemble inference (Multi-key exact + Fuzzy Train-Lookup + SMT + NLLB + MBR Union)...")
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df)):
        ...
        # 1) Multi-key exact-match lookup từ train
        hyp_exact = lookup_train_exact(src_sent, src_lang_c, tgt_lang_c)
        if hyp_exact is not None and str(hyp_exact).strip():
            preds.append(hyp_exact)   # KHÔNG postprocess để giữ y nguyên train
            exact_hits += 1
            continue
        ...
        if fuzzy_hyp is not None and str(fuzzy_hyp).strip():
            preds.append(fuzzy_hyp)  # KHÔNG postprocess
            fuzzy_hits += 1
            continue
        ...
        if not pool:
            ...
            best = postprocess_target(best, tgt_lang_c)
...
2025-11-13 22:18:58.377681: E external/local_xla/xla/stream_executor/cuda/cuda_fft.cc:477] Unable to register cuFFT factory: Attempting to register factory for plugin cuFFT when one has already been registered
WARNING: All log messages before absl::InitializeLog() is called are written to STDERR
E0000 00:00:1763072338.785274      19 cuda_dnn.cc:8310] Unable to register cuDNN factory: Attempting to register factory for plugin cuDNN when one has already been registered
E0000 00:00:1763072338.883864      19 cuda_blas.cc:1418] Unable to register cuBLAS factory: Attempting to register factory for plugin cuBLAS when one has already been registered
...
Creating train lookup dictionary (for multi-key exact + fuzzy copy)...
...
Exact lookup example: Hindi->Bhilli has 43,323 keys.
...
🚀 Ensemble inference (Multi-key exact + Fuzzy Train-Lookup + SMT + NLLB + MBR Union)...
100%
 15999/15999 [5:08:16<00:00, 823.42it/s]
✅ Wrote final ENSEMBLE submission to: /kaggle/working/submission.csv
✅ Exact train-copy hits : 3560
✅ Fuzzy train-copy hits : 0
...
3	26313	Hindi	श्री मोदी ने कहा यह अटल जी ही थे जिन्होंने देश...	Bhilli	श्री मोदी यी केदु इद अटल जी ही थी जिनो देश नी ...
4	83303	Hindi	उत्सवादि मनाने के उपलक्ष्य में सुरापान करना सा...	Bhilli	उत्सवादि मनवा ना उपलक्ष्य मा सुरापान करवा आम -...
5	131411	Hindi	तुम्‍हारे साथ कभी ऐसा हुआ।	Bhilli	तमुन संग कदी इदेक आयी।
6	101809	Hindi	यह सत्र ग्लासगो, यूनाइटेड किंगडम में आयोजित हुआ।	Bhilli	इद सत्र ग्लासगो युनायटेड किंगडम ते आयोजित आयर।
7	59328	Hindi	यह 9 मार्च 2012 को रिलीज़ हुई थी, जिसे आम तौर ...	Bhilli	ये 9 मार्च 2012 को रिलीज़ हुई थी, जिनकी आम तौर...
...
```
2025-11-13 06:38:50.439839: E external/local_xla/xla/stream_executor/cuda/cuda_fft.cc:477] Unable to register cuFFT factory: Attempting to register factory for plugin cuFFT when one has already been registered
WARNING: All log messages before absl::InitializeLog() is called are written to STDERR
E0000 00:00:1763015930.575458      19 cuda_dnn.cc:8310] Unable to register cuDNN factory: Attempting to register factory for plugin cuDNN when one has already been registered
E0000 00:00:1763015930.615544      19 cuda_blas.cc:1418] Unable to register cuBLAS factory: Attempting to register factory for plugin cuBLAS when one has already been registered
```
...
Loaded 20,000 pairs: Hindi <-> Mundari
Loaded 20,000 pairs: English <-> Santali
Lookup example: Hindi->Bhilli has 19,575 entries.
Loading test file from: /kaggle/input/mm-lo-so-2025/test.csv
Loading Tokenizer from ADAPTER_PATH...
Loading Base NLLB model...
Attaching LoRA adapter...
...
🚀 Ensemble inference (Train-Lookup + SMT + NLLB + MBR Union)...
```

100%

 15999/15999 [4:48:52<00:00, 704.35it/s]
