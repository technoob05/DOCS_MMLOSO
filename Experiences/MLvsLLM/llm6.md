Private Score : 166.46617

Public Score :  302.08483
...
# fix seed một chút cho sampling ổn định hơn
np.random.seed(42)
...
# ============================================================
# 3. TRAIN-LOOKUP (COPY-FROM-TRAIN)
# ============================================================
...
for (s_lang, t_lang), pairs in bitext_by_dir.items():
    d = train_lookup[(s_lang, t_lang)]
    for s, t in pairs:
        d[s] = t

print(f"Lookup example: Hindi->Bhilli has {len(train_lookup[('Hindi','Bhilli')]):,} entries.")
...
# Config NLLB decoding + sampling
NLLB_BEAM_SIZE    = 8   # num_beams cho beam-search
NLLB_BEAM_N_BEST  = 8   # số candidate lấy từ beam-search
NLLB_SAMP_N_BEST  = 4   # số candidate từ sampling
NLLB_TOP_K        = 50
NLLB_TOP_P        = 0.9
NLLB_TEMPERATURE  = 0.8
...
# ============================================================
# 8. NLLB GENERATION – BEAM N-BEST + SAMPLING N-BEST
# ============================================================
@torch.no_grad()
defnllb_generate_n_best(texts, src_lang, tgt_lang, max_new_tokens=256):
"""
    Sinh candidate từ NLLB:
      - Beam search:
          num_beams = NLLB_BEAM_SIZE
          num_return_sequences = NLLB_BEAM_N_BEST
      - Sampling:
          do_sample = True, top_k / top_p / temperature
          num_return_sequences = NLLB_SAMP_N_BEST

    Trả về: list string (beam + sampling), đã normalize, chưa unique.
    """
    ...
    # ---------- (a) Beam search N-best ----------
    try:
        beam_tokens = model.generate(
            **inputs,
        ...
        beam_outs = tokenizer.batch_decode(beam_tokens, skip_special_tokens=True)
        for o in beam_outs:
            s = normalize_space(o)
            if s:
                all_outs.append(s)
    except Exception as e:
        print("⚠️ Beam generation error:", e)

    # ---------- (b) Sampling N-best ----------
    try:
        samp_tokens = model.generate(
            **inputs,
            max_new_tokens=max_new_tokens,
            do_sample=True,
            ...
            top_k=NLLB_TOP_K,
            top_p=NLLB_TOP_P,
            temperature=NLLB_TEMPERATURE,
            no_repeat_ngram_size=3,
            forced_bos_token_id=forced_bos_id
        )
        samp_outs = tokenizer.batch_decode(samp_tokens, skip_special_tokens=True)
        for o in samp_outs:
            s = normalize_space(o)
            if s:
                all_outs.append(s)
    except Exception as e:
        print("⚠️ Sampling generation error:", e)

    return all_outs

# ============================================================
# 9. ENSEMBLE TRANSLATION (TRAIN-LOOKUP + SMT + NLLB + MBR)
# ============================================================
os.environ["TOKENIZERS_PARALLELISM"] = "false"

if USE_NLLB or USE_SMT:
    preds = []
    print("\n🚀 Ensemble inference (Train-Lookup + SMT + NLLB (beam+sampling) + MBR Union)...")
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df)):
        ...
        if src_norm in train_lookup.get((src_lang_c, tgt_lang_c), {}):
            hyp = train_lookup[(src_lang_c, tgt_lang_c)][src_norm]
...
```
2025-11-13 12:32:09.738377: E external/local_xla/xla/stream_executor/cuda/cuda_fft.cc:477] Unable to register cuFFT factory: Attempting to register factory for plugin cuFFT when one has already been registered
WARNING: All log messages before absl::InitializeLog() is called are written to STDERR
E0000 00:00:1763037129.923240      19 cuda_dnn.cc:8310] Unable to register cuDNN factory: Attempting to register factory for plugin cuDNN when one has already been registered
E0000 00:00:1763037129.972692      19 cuda_blas.cc:1418] Unable to register cuBLAS factory: Attempting to register factory for plugin cuBLAS when one has already been registered
```
...
 15999/15999 [9:16:41<00:00, 356.78it/s]
...
| 1 | 87641  | Hindi       | वित्तीय कठिनाइयों को हल करने में सहायक होने के...   | Bhilli      | आर्थिक कठिनाइयों ने हल करवा मा मदद ना हाते-हात...      |
| 2 | 32543  | Hindi       | मेरा सुझाव है कि हमारे सक्रिय दृष्टिकोण, नीतिय...   | Bhilli      | अइञ सुझाव मनअः कि मावा सक्रिय दृष्टिकोण नीतीज ...     |
| 3 | 26313  | Hindi       | श्री मोदी ने कहा यह अटल जी ही थे जिन्होंने देश...     | Bhilli      | श्री मोदी यी केदु इद अटल जी ही थाये जिनह देह न...        |
| 4 | 83303  | Hindi       | उत्सवादि मनाने के उपलक्ष्य में सुरापान करना सा...  | Bhilli      | इल् ल्तम मदन हानाना नुंग उदेक ते सुरापान करानु...     |
| 5 | 131411 | Hindi       | तुम्‍हारे साथ कभी ऐसा हुआ।                                          | Bhilli      | तमुन संग कुबैय इदेक आयता।                                              |
| 6 | 101809 | Hindi       | यह सत्र ग्लासगो, यूनाइटेड किंगडम में आयोजित हुआ।  | Bhilli      | इद सत्र ग्लासगो युनाइटेड किंगडम ते आयोजित थायतोर। |
| 7 | 59328  | Hindi       | यह 9 मार्च 2012 को रिलीज़ हुई थी, जिसे आम तौर ...            | Bhilli      | इद 9 मार्च 2012 ने रिलीज थाई , जिना सामान्य रू...             |
| 8 | 57205  | Hindi       | नियुक्ति मामलों की मंत्रिमंडलीय समिति ने भारती... | Bhilli      | नियुक्ति मामलों नी मंत्रिमंडलीय समिति ने भारत ...    |
| 9 | 103641 | Hindi       | भारत को और अधिक साफ-सुथरा बनाने और बेहतर स्वच्...    | Bhilli      | भारत ते बीजो वदु साफ-सुथरा बणावा अन बेसे साफ-स...       |
