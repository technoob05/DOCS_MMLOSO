Private Score:173.57974
Public Score :306.11032
...
print("Cài đặt/Kiểm tra sacrebleu, safetensors và protobuf...")
_ = _pip_install(["sacrebleu", "safetensors", "protobuf<5"])
...
# Base NLLB model như cũ
MODEL_INPUT_DIR = "/kaggle/input/number-1-noone-canbehere"
NLLB_BASE_PATH  = os.path.join(MODEL_INPUT_DIR, "nllb200-600m")

# Adapter gốc (đã dùng để submit trước đó)
ADAPTER_BASE_DIR = os.path.join(MODEL_INPUT_DIR, "nllb-lora-adapter-final")

# 🆕 Chọn checkpoint mới từ finetune-mmoloso
FINETUNE_ROOT        = "/kaggle/input/finetune-mmoloso/Finetune_mmoloso"
LORA_CHECKPOINT_STEP = "checkpoint-10500"  # ← đổi thành 'checkpoint-10000' hoặc 'checkpoint-10551' nếu muốn
LORA_CKPT_DIR        = os.path.join(
    FINETUNE_ROOT,
    "nllb-lora-tuning-checkpoints",
    LORA_CHECKPOINT_STEP,
)
...
print(f"Loading Base LoRA Adapter from: {ADAPTER_BASE_DIR}")
print(f"LoRA checkpoint weights from  : {LORA_CKPT_DIR}")
...
fuzzy_lookup  = defaultdict(lambda: defaultdict(list))
...
# ============================================================
# 5. LOAD NLLB + LORA (BASE + OVERRIDE CHECKPOINT)
# ============================================================
NLLB_LANG_CODE = {
    "English": "eng_Latn",
    "Hindi":   "hin_Deva",
...
try:
    print("Loading Tokenizer from NLLB_BASE_PATH...")
    tokenizer = AutoTokenizer.from_pretrained(NLLB_BASE_PATH, use_fast=True)
    ...
    # 1) Gắn adapter gốc (có adapter_config.json)
    print(f"Attaching base LoRA adapter from: {ADAPTER_BASE_DIR}")
    model = PeftModel.from_pretrained(model, ADAPTER_BASE_DIR)

    # 2) Nạp lại trọng số từ checkpoint (adapter_model.safetensors)
    ckpt_file = os.path.join(LORA_CKPT_DIR, "adapter_model.safetensors")
    if os.path.exists(ckpt_file):
        print(f"Loading LoRA weights from checkpoint: {ckpt_file}")
        sd = load_file(ckpt_file)
        missing, unexpected = model.load_state_dict(sd, strict=False)
        print(f"✅ Loaded checkpoint weights. Missing keys: {len(missing)}, unexpected: {len(unexpected)}")
    else:
        print(f"⚠️ Không tìm thấy {ckpt_file}, dùng nguyên adapter gốc (không override checkpoint).")

    print(f"✅ NLLB + LoRA ({LORA_CHECKPOINT_STEP}) ready for inference.")
...
```
Cài đặt/Kiểm tra sacrebleu, safetensors và protobuf...
🌐 pip install ['sacrebleu', 'safetensors', 'protobuf<5']
...
2025-11-14 12:26:47.583180: E external/local_xla/xla/stream_executor/cuda/cuda_fft.cc:477] Unable to register cuFFT factory: Attempting to register factory for plugin cuFFT when one has already been registered
WARNING: All log messages before absl::InitializeLog() is called are written to STDERR
E0000 00:00:1763123207.774653      19 cuda_dnn.cc:8310] Unable to register cuDNN factory: Attempting to register factory for plugin cuDNN when one has already been registered
E0000 00:00:1763123207.827923      19 cuda_blas.cc:1418] Unable to register cuBLAS factory: Attempting to register factory for plugin cuBLAS when one has already been registered
```

```
Sacrebleu loaded, version: 2.5.1
Loading Base Model from       : /kaggle/input/number-1-noone-canbehere/nllb200-600m
Loading Base LoRA Adapter from: /kaggle/input/number-1-noone-canbehere/nllb-lora-adapter-final
LoRA checkpoint weights from  : /kaggle/input/finetune-mmoloso/Finetune_mmoloso/nllb-lora-tuning-checkpoints/checkpoint-10500
...
Loading Tokenizer from NLLB_BASE_PATH...
Loading Base NLLB model...
Attaching base LoRA adapter from: /kaggle/input/number-1-noone-canbehere/nllb-lora-adapter-final
⚠️ Không tìm thấy /kaggle/input/finetune-mmoloso/Finetune_mmoloso/nllb-lora-tuning-checkpoints/checkpoint-10500/adapter_model.safetensors, dùng nguyên adapter gốc (không override checkpoint).
✅ NLLB + LoRA (checkpoint-10500) ready for inference.
